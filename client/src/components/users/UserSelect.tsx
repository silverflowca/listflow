  ...
  return (
    <div ref={ref} className={cn('relative', className)}>
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-2 px-3 py-2 rounded-lg border border-ios-gray-5 bg-white text-left text-sm ws-focus"
      >
        {selected.length === 0 ? (
          ...
          <ul className="max-h-52 overflow-y-auto py-1">
            ...
            {filtered.map(u => (
              ...
              >
                ...
                {value.includes(u.id) && <span className="ws-text text-xs">✓</span>}
              </li>
            ))}
          </ul>
        </div>
