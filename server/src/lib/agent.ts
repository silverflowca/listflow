...
const LISTFLOW_TOOLS: Anthropic.Tool[] = [
  {
    name: 'create_task',
...
async function executeTool(
  toolName: string,
  input: Record<string, unknown>,
  ctx: { workspaceId: string; createdBy: string; runId: string },
  tasksCreated: string[],
): Promise<{ ok: boolean; result: unknown }> {
  try {
    switch (toolName) {
      case 'create_task': {
        const { data, error } = await (lf('tasks') as any).insert({
      ...
      case 'create_page': {
        const { data: page, error: pageErr } = await (lf('pages') as any).insert({
          workspace_id: ctx.workspaceId,
          title: input.title as string,
          created_by: ctx.createdBy,
        }).select().single()

        if (pageErr) return { ok: false, result: { error: pageErr.message } }

        if (input.content) {
          await (lf('blocks') as any).insert({
            page_id: page.id,
            type: 'text',
            content: { text: input.content as string },
            position: 0,
          })
        }
      ...
      case 'summarize':
        return { ok: true, result: { summary: input.summary as string } }

      case 'remember': {
        await (lf('agent_memory') as any).upsert(
          { workspace_id: ctx.workspaceId, key: input.key as string, value: input.value as string },
      ...
      case 'recall': {
        const { data } = await (lf('agent_memory') as any)
          .select('value')
...
export async function runAgent(opts: AgentRunInput): Promise<AgentRunOutput> {
  ...
  if (!anthropicKey && !geminiKey) {
    ...
    await (lf('agent_runs') as any).update({ status: 'failed', response: errMsg, completed_at: new Date().toISOString() }).eq('id', runId)
  ...
  try {
    for (iteration = 0; iteration < maxIterations; iteration++) {
      ...
      for (const block of toolUseBlocks) {
        ...
        const { ok, result } = await executeTool(
          ...
          { workspaceId, createdBy, runId },
          tasksCreated,
        )
        const durationMs = Date.now() - start

        allToolCalls.push({ toolName: block.name, input: block.input as Record<string, unknown>, ok, result, durationMs })
    ...
    await (lf('agent_runs') as any).update({
      status: 'done',
...
async function runAgentGemini(opts: AgentRunInput & {
  geminiKey: string
  tasksCreated: string[]
...
`
    ...
    // Parse JSON response
    let parsed: { tasks?: Array<{ title: string; description?: string; priority?: string }>; pages?: Array<{ title: string; content?: string }>; summary?: string }
    try {
    ...
    for (const page of parsed.pages ?? []) {
      const { data } = await (lf('pages') as any).insert({
        workspace_id: workspaceId,
        title: page.title,
        created_by: createdBy,
      }).select().single()
      ...
      if (data && page.content) {
        await (lf('blocks') as any).insert({ page_id: data.id, type: 'text', content: { text: page.content }, position: 0 })
      }
    }
