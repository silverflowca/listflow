...
async function getToken(baseUrl: string, email: string, password: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(`FileFlow login failed: ${res.status} ${await res.text()}`)
...
export async function mirrorToFileFlow(opts: {
  ...
  workspaceId: string
  listflowRecordingId: string
  listflowTranscriptId: string | null
}): Promise<FileFlowFile | null> {
  ...
  if (!urlRes.ok) throw new Error(`FileFlow upload-url failed: ${urlRes.status}`)
  ...
  const putRes = await fetch(uploadUrl, {
    ...
    body: opts.audioBuffer as unknown as BodyInit,
  })
  if (!putRes.ok) throw new Error(`FileFlow storage PUT failed: ${putRes.status}`)
  ...
  if (!fileRes.ok) throw new Error(`FileFlow create file record failed: ${fileRes.status}`)
  const fileRecord = await fileRes.json() as FileFlowFile
  ...
  if (opts.transcript) {
    try {
      ...
      if (txtUrlRes.ok) {
        const { uploadUrl: txtUrl, storagePath: txtPath } = await txtUrlRes.json() as { uploadUrl: string; storagePath: string }
        const txtPut = await fetch(txtUrl, { method: 'PUT', headers: { 'Content-Type': 'text/plain' }, body: txtBuffer as unknown as BodyInit })
 ...
 */
export async function testFileFlowConnection(baseUrl: string, email: string, password: string): Promise<{ ok: boolean; error?: string }> {
  try {
    invalidateToken()
