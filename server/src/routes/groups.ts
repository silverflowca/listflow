...
r.get('/', async (c) => {
  const { data, error } = await (lf('user_groups') as any)
...
r.post('/', async (c) => {
  ...
  const { data, error } = await (lf('user_groups') as any).insert(body).select().single()
  if (error) return c.json({ error: error.message }, 500)
...
// ── Delete group ──────────────────────────────────────────────────────────────
r.delete('/:id', async (c) => {
  const { error } = await (lf('user_groups') as any).delete().eq('id', c.req.param('id'))
  if (error) return c.json({ error: error.message }, 500)
...
r.post('/:id/members', async (c) => {
  const body = await c.req.json()
  const { data, error } = await (lf('group_members') as any)
...
r.delete('/:id/members/:userId', async (c) => {
  const { error } = await (lf('group_members') as any)
    .delete()
