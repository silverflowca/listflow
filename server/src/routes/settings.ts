...
import { getConfig, setConfig, deleteConfig, SENSITIVE_KEYS, configCache } from '../lib/config.js'
...
r.post('/fileflow/test', requireAuth, async (c) => {
  const body = await c.req.json() as { url?: string; email?: string; password?: string }
  ...
  if (!url || !email || !password) return c.json({ ok: false, error: 'URL, email and password are required' }, 400)
...
r.patch('/:key', requireAuth, async (c) => {
  ...
  if (!setting) return c.json({ error: `Unknown setting: ${key}` }, 400)
...
r.delete('/:key', requireAuth, async (c) => {
  ...
  const envValue = (process.env as Record<string, string | undefined>)[key] ?? undefined
  const setting = ALL_SETTINGS.find(s => s.key === key)
