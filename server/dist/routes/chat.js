...
r.get('/channels', requireAuth, async (c) => {
    ...
    let { data: channels, error } = await lf('chat_channels')
        .select('*')
...
r.post('/channels', requireAuth, async (c) => {
    ...
    const { data, error } = await lf('chat_channels')
        .insert({
...
r.post('/channels/:id/messages', requireAuth, async (c) => {
    ...
    const { data: channel, error: chErr } = await lf('chat_channels')
        .select('workspace_id')
        .eq('id', channelId)
        .single();
    if (chErr || !channel)
        return c.json({ error: 'Channel not found' }, 404);
    const { data: msg, error } = await lf('chat_messages')
        .insert({
...
r.patch('/channels/:id/messages/:msgId', requireAuth, async (c) => {
    ...
    const { data: existing, error: fetchErr } = await lf('chat_messages')
        .select('*')
        .eq('id', msgId)
        .single();
    ...
    const { data: updated, error } = await lf('chat_messages')
    ...
    if (error)
        return c.json({ error: error.message }, 500);
    emitChatMessageUpdated(updated);
...
r.delete('/channels/:id/messages/:msgId', requireAuth, async (c) => {
    ...
    if (scope === 'everyone') {
        ...
        if (existing.user_id !== user.id)
            return c.json({ error: 'Not your message' }, 403);
        const { error } = await lf('chat_messages')
        ...
        const deletedFor = Array.isArray(existing.deleted_for) ? existing.deleted_for : [];
...
r.post('/channels/:id/pin-task', requireAuth, async (c) => {
    ...
    const { data: msg, error: msgErr } = await lf('chat_messages')
        .select('*')
    ...
    // Create task
    const { data: task, error: taskErr } = await lf('tasks')
        .insert({
        workspace_id: body.workspaceId,
        title: titleText,
