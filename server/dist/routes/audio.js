...
r.post('/upload', requireAuth, async (c) => {
    ...
    const pageId = formData.get('pageId') ?? undefined;
    const taskId = formData.get('taskId') ?? undefined;
    ...
    const { data: recording, error: recErr } = await lf('audio_recordings')
        .insert({
    ...
    try {
        ...
        const { data: transcript } = await lf('transcripts')
            .insert({
        ...
        if (transcript) {
            ...
            mirrorToFileFlow({
                ...
                if (ff)
                    console.log(`[fileflow] Mirrored recording ${recording.id} → FileFlow file ${ff.id}`);
    ...
    if (rawText.length > 10 && transcriptId) {
        const { data: run } = await lf('agent_runs')
            .insert({
            workspace_id: workspaceId,
...
r.get('/task/:taskId', requireAuth, async (c) => {
    ...
    const { data, error } = await lf('audio_recordings')
...
r.delete('/:id', requireAuth, async (c) => {
    ...
    const { error: delErr } = await lf('audio_recordings')
        .delete()
        .eq('id', c.req.param('id'));
    if (delErr)
