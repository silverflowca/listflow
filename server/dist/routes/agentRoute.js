...
r.post('/run', requireAuth, async (c) => {
    ...
    const { data: run, error: runErr } = await lf('agent_runs')
        .insert({
        ...
        .single();
    if (runErr)
        return c.json({ error: runErr.message }, 500);
    // Run agent async
    runAgent({
        runId: run.id,
...
r.get('/runs', requireAuth, async (c) => {
    ...
    const { data, error } = await lf('agent_runs')
        .select('*')
...
r.get('/memory', requireAuth, async (c) => {
    ...
    if (!workspaceId)
        return c.json({ error: 'workspaceId required' }, 400);
    const { data, error } = await lf('agent_memory')
        .select('*')
        .eq('workspace_id', workspaceId)
...
r.delete('/memory/:key', requireAuth, async (c) => {
    const workspaceId = c.req.query('workspaceId');
    const { error } = await lf('agent_memory')
        .delete()
        .eq('workspace_id', workspaceId)
