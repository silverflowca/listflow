...
// ── List all app users ────────────────────────────────────────────────────────
r.get('/', async (c) => {
    const { data, error } = await lf('app_users')
        .select('*')
        .order('name');
    if (error)
...
// ── Get single user ───────────────────────────────────────────────────────────
r.get('/:id', async (c) => {
    const { data, error } = await lf('app_users').select('*').eq('id', c.req.param('id')).single();
    if (error)
...
r.post('/', async (c) => {
    ...
    const { data, error } = await lf('app_users').insert(body).select().single();
    if (error)
        return c.json({ error: error.message }, 500);
...
r.delete('/:id', async (c) => {
    ...
    const { error } = await lf('app_users')
