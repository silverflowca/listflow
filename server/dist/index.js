...
app.get('/share', async (c) => {
    ...
    const { data: tasks } = await lf('tasks')
    ...
    const { data: workspaces } = await lf('workspaces')
        .select('id, name')
        .in('id', wsIds);
    const wsMap = {};
    ...
    const lines = (tasks ?? []).map((t) => {
        const wsName = wsMap[t.workspace_id] ?? '';
        const initials = wsName.trim().split(/\s+/).map((w) => w[0]?.toUpperCase() ?? '').join('').slice(0, 3);
...
<body>
  <p style="font-family:sans-serif;padding:2rem">${lines.map((l) => `<strong>${l}</strong>`).join('<br/>')}</p>
